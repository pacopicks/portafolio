...
export default function SocialCards({ cards }: SocialCardsProps) {
  ...
  const cardSizeClasses =
    "w-[8.5rem] h-[13rem] sm:w-[10rem] sm:h-[15rem] md:w-[11rem] md:h-[16rem] lg:w-[14rem] lg:h-[20rem] xl:w-[16rem] xl:h-[22rem] rounded-2xl shadow-xl";

  return (
    <section className="flex flex-col items-center w-full py-4 lg:py-8 px-4 md:px-8 relative z-20">
      <div className="flex items-center justify-center w-full max-w-[90rem] h-[22rem] sm:h-[26rem] md:h-[28rem] lg:h-[34rem] xl:h-[38rem]">
        <div ref={containerRef} className="fan-layout flex relative justify-center items-center w-full max-w-[80rem] h-full">
          {cards.map((card, index) => {
            ...
            return card.linkUrl ? (
              <a key={index} href={card.linkUrl} target={card.linkUrl.startsWith("http") ? "_blank" : "_self"} rel="noopener noreferrer" className={`fan-card absolute block cursor-pointer ${cardSizeClasses}`}>{image}</a>
            ) : (
              <div key={index} className={`fan-card absolute ${cardSizeClasses}`}>{image}</div>
            );
          })}
        </div>
      </div>
