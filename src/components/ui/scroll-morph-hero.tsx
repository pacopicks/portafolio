    ...
    return (
        ...
        >
            ...
            >
                ...
                {/* Back Face */}
                <div
                    className="absolute inset-0 h-full w-full overflow-hidden rounded-xl shadow-lg bg-neutral-900 flex flex-col items-center justify-center p-4 border border-neutral-700"
                    style={{ backfaceVisibility: "hidden", transform: "rotateY(180deg)" }}
                >
                    <div className="text-center">
                        <p className="text-[8px] font-bold text-blue-400 uppercase tracking-widest mb-1">Ver</p>
                        <p className="text-xs font-medium text-white">Detalle</p>
                    </div>
                </div>
            </motion.div>
        </motion.div>
...
const MAX_SCROLL = 1000; // Virtual scroll range (kept short so the section releases the page scroll quickly)

const IMAGES = Array.from({ length: TOTAL_IMAGES }, (_, i) => {
    const shades = ["1a1a1a", "242424", "2b2b2b", "333333", "3c3c3c"];
    const color = shades[i % shades.length];
    return `https://placehold.co/300x420/${color}/888888/png?text=${i + 1}`;
});
...
export default function IntroAnimation() {
    ...
    useEffect(() => {
        ...
        const handleWheel = (e: WheelEvent) => {
            // Once the internal animation has run its course in the
            // scroll direction, let the event pass through so the page
            // keeps scrolling normally instead of staying pinned here.
        ...
        const handleTouchMove = (e: TouchEvent) => {
            ...
            const atMax = scrollRef.current >= MAX_SCROLL;
            const atMin = scrollRef.current <= 0;
            if ((deltaY > 0 && atMax) || (deltaY < 0 && atMin)) {
                return;
            }
    ...
    // --- Random Scatter Positions (computed once per mount) ---
            ...
            <div className="flex h-full w-full flex-col items-center justify-center perspective-1000">
                ...
                >
                    ...
                    <p className="text-sm md:text-base text-muted-foreground max-w-lg leading-relaxed">
                        Un recorrido por distintas series y proyectos. <br className="hidden md:block" />
                        Sigue desplazándote para descubrir más trabajo.
                    </p>
                </motion.div>
